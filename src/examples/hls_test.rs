            ...
            Ok(resp) => {
                ...
                if resp.status().is_success() {
                    ...
                    if let Some(url) = variant_url {
                        println!("\n[INFO] 2. 获取码率播放列表");
                        let proxy_variant_url = format!("http://127.0.0.1:8080/proxy/{}", url);
                        
                        match client.get(&proxy_variant_url).send().await {
                            Ok(resp) => {
                                println!("[INFO] 状态码: {}", resp.status());
                                if resp.status().is_success() {
                                    let content = resp.text().await?;
                                    ...
                                    if let Some(url) = segment_url {
                                        println!("\n[INFO] 3. 下载视频分片");
                                        let proxy_segment_url = format!("http://127.0.0.1:8080/proxy/{}", url);
                                        
                                        // 3.1 完整下载
                                        println!("[INFO] 3.1 完整下载分片");
                                        match client.get(&proxy_segment_url).send().await {
                                            Ok(resp) => {
                                                println!("[INFO] 状态码: {}", resp.status());
                                        ...
                                        // 3.2 范围请求
                                        println!("\n[INFO] 3.2 范围请求分片");
                                        match client.get(&proxy_segment_url)
